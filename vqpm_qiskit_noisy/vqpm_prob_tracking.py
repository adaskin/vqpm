...
def vqpm_with_prob_tracking(u, n, max_iter=30, target_state=None, pdiff=0.01, precision=3):
    p_min = np.zeros(max_iter)
    N = 2**n
    s2 = 1 / np.sqrt(2)
    ...
    for j in range(max_iter):
        ...
        # Calculate qubit probabilities for this iteration
        iteration_probs = {}
        for qubit in range(1, n + 1):
            if qubit in q_states:
                iteration_probs[qubit] = q_states[qubit]
            else:
                prob_vec = calculate_prob(psi0_final, qubit)
                iteration_probs[qubit] = np.round(prob_vec, precision)
        
        qubit_prob_history.append(iteration_probs)
        
        # Prepare new state (locking logic)
        state = [complex(1)]
        for q in range(1, n + 1):
            if q in q_states:
                state_q = q_states[q]
            else:
                state_q = iteration_probs[q]
                ...
                if state_q[0] > state_q[1] + pdiff:
                    ...
                    state_q = np.sqrt(state_q)
            
            state = np.kron(state, state_q)
        
        in_vec = state
        
        # Track target probability for early stopping
        p_min[j] = np.abs(psi0_final[target_state]) ** 2
        if p_min[j] >= 0.5 or (p_min[j] == 0 and j > 5):
            num_iter = j + 1
            break
    ...
    return found_state, max_prob, q_states, num_iter, p_min[:num_iter], qubit_prob_history
